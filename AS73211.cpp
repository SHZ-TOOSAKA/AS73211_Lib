...
AS73211::AS73211(uint8_t slave){       //传入从机地址
  _slaveAddress =slave;
...
bool AS73211::setState(OperationalState state){         //设置state，之后从传感器读回state作为返回数据
	delay(500);
...
uint16_t AS73211::getGainValue(){         //获取当前增益设置
	switch(getGain()){
...
uint16_t AS73211::getTimeValue(){         //积分时间设置
	switch(getTime()){
...
float AS73211::getTemperature()           // check，得到温度大致如19.05
{
...
void AS73211::softwareReset()         //软重启
{
    setState(Software_reset);
	delay(500);
}

float AS73211::convertingToEe(uint8_t channel, uint16_t MRES_data)   //数据转换函数：第一个为设置X、Y、Z channel，第二个：为初始值
{
    float dataValue;
    uint8_t cnt;
    ...
    for (cnt = 0; cnt < 12; cnt++)
    {
        if (_channel_gain[ cnt ] == _setGain)
        {
             ...
             {
                 _FSR = _X_channel_FSR[ cnt ];                  //_FSR :累加____
             }
